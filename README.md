...
|     location      | **谨慎修改该变量**，该变量存储不同分辨率下标识截图的坐标信息 |
...
- 2024-04-18
  成就全集文件迭代，由2.0版本迭代至2.1版本，并且完成项目对1920*1080类型设备的匹配。同时完成项目多线程实现强制停止操作
- 2024-05-19
  成就全集文件迭代，由2.1版本迭代至2.2版本。同时修复了由于github不允许空文件夹上传以及创建导致的路径图片数量检测未通过的问题，修改了README中关于imgs文件夹架构的表述。
  （此处感谢@LUX发现了对应的bug）
